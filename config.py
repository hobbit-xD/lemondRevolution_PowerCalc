...
try:
    if MAIN_DEBUG: print 'Open file ' + _CONFIG_FILENAME
    file = open(_CONFIG_FILENAME, 'rb')
    if MAIN_DEBUG: print 'Parse config'
    CONFIG.readfp(file)
except Exception as e:
    print "Error: "+repr(e.__class__)

if MAIN_DEBUG: print 'Get config items'

# Type of sensor connected to the trainer
SPEED_SENSOR_TYPE = CONFIG.getint(SECTION, 'speed_sensor_type')
CADENCE_SENSOR_TYPE = CONFIG.getint(SECTION, 'cadence_sensor_type')
...
#POWER_CALCULATOR.set_debug(DEBUG or MAIN_DEBUG)
